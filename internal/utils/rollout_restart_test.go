					...
					case *appsv1.DeploymentList:
						for _, obj := range objType.Items {
							...
							if gotObj.Spec.Template.Annotations == nil {
								t.Fatalf("HandleRolloutRestartList() expected %s to have annotation %s", obj.Name, constants.RolloutRestartAnnotation)
							}
							if _, ok := gotObj.Spec.Template.Annotations[constants.RolloutRestartAnnotation]; !ok {
								t.Fatalf("HandleRolloutRestartList() expected %s to have annotation %s", obj.Name, constants.RolloutRestartAnnotation)
							}
							if gotObj.Annotations == nil {
								t.Fatalf("HandleRolloutRestartList() expected %s to have annotation %s", obj.Name, constants.RolloutManagedBy)
							}
							if gotVal, ok := gotObj.Annotations[constants.RolloutManagedBy]; !ok || gotVal != tt.args.flipperNamespacedName {
								t.Fatalf("HandleRolloutRestartList() expected %s to have annotation %s with value %s but got %s", obj.Name, constants.RolloutManagedBy, tt.args.flipperNamespacedName, gotVal)
							}
